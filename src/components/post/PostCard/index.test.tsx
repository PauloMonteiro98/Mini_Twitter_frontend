...
const mockPost: Post = {
    ...
    likesCount: 10,
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSswwvA9WWb2e3ZfFOEMZHik4jQ9MgJGCEDbg&s',
...
describe('Componente PostCard', () => {
  ...
  it('deve implementar curtida otimista: aumenta contador instantaneamente e chama API', async () => {
    vi.mocked(api.post).mockResolvedValueOnce({ data: { success: true } });
    
    render(<PostCard post={mockPost} />);
    
    const heartButton = screen.getByRole('button', { name: /10/ });
  ...
  it('deve reverter o contador (rollback) se a API de like falhar', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('10')).toBeInTheDocument();
      expect(heartButton.querySelector('svg')).not.toHaveClass('fill-[#EB5757]');
    });
  });
  ...
  it('deve entrar no modo de edição e exibir campos de input ao clicar em editar', async () => {
    ...
    expect(screen.getByPlaceholderText('Editando...')).toHaveValue(mockPost.content);
  ...
  it('deve permitir cancelar a edição e restaurar valores originais', async () => {
    ...
    expect(screen.queryByPlaceholderText('Editando...')).not.toBeInTheDocument();
